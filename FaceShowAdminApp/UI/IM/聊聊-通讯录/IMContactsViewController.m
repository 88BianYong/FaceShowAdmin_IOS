...
- (void)requestClasses {
    ...
    [self.getClassRequest startRequestWithRetClass:[ClassListRequestItem class] andCompleteBlock:^(id retItem, NSError *error, BOOL isMock) {
        ...
        self.classFilterView.dataArray = self.clazsListItem.data.clazsInfos;
        ...
        [self.clazsListItem.data.clazsInfos enumerateObjectsUsingBlock:^(ClassListRequestItem_clazsInfos *obj, NSUInteger idx, BOOL * _Nonnull stop) {
            if ([obj.clazsId isEqualToString:[UserManager sharedInstance].userModel.currentClass.clazsId]) {
...
- (void)showFilterView {
    ...
    [filterView setClazsListFilterCompleteBlock:^(ClassListRequestItem_clazsInfos *selectedClass, NSInteger selectedRow) {
        STRONG_SELF
        [alert hide];
        if (![selectedClass.clazsId isEqualToString:self.selectedClass.clazsId]) {
